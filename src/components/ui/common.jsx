...
export const TitleComponent = ({
  ...
  return (
    <div className={`${className} section-title`}>
      ...
      <h2 className={`${h2_class} mt-2 display-6 fw-semiboldmb-2`}>{title}</h2>
    ...
    <div className="search-fleet-container position-relative z-3">
      <form className="search-fleet-card shadow-sm" onSubmit={handleSearch}>
        ...
        {(isVehiclePage || isHotelPage) && (
          <div
            className={`filter-group ${openDropdown === "category" ? "raise-z-index" : ""}`}
          >
            ...
            <div className="input-wrapper position-relative">
              ...
              >
                ...
                <ChevronDown
                  size={18}
                  className={`select-arrow-transition ${openDropdown === "category" ? "rotate-arrow" : ""}`}
                />
              </div>

              <div
                className={`custom-dropdown-options-box shadow-lg ${openDropdown === "category" ? "open-expanded" : ""}`}
              >
                <div
                  className="option-item"
                  onClick={() => handleSelectOption("category", "all")}
                >
                  {t("categories.all")}
                </div>
                {isVehiclePage && (
                  <>
                    <div
                      className="option-item"
                      onClick={() => handleSelectOption("category", "Sedan")}
                    >
                      {t("categories.Sedan")}
                    </div>
                    <div
                      className="option-item"
                      onClick={() => handleSelectOption("category", "SUV")}
                    >
                      {t("categories.SUV")}
                    </div>
                    <div
                      className="option-item"
                      onClick={() =>
                        handleSelectOption("category", "Traveller")
                      }
                    >
                      {t("categories.Traveller")}
                    </div>
                  </>
                )}
                {isHotelPage && (
                  <>
                    <div
                      className="option-item"
                      onClick={() =>
                        handleSelectOption("category", "singleRoom")
                      }
                    >
                      {t("categories.singleRoom")}
                    </div>
                    <div
                      className="option-item"
                      onClick={() =>
                        handleSelectOption("category", "doubleRoom")
                      }
                    >
                      {t("categories.doubleRoom")}
                    </div>
                    <div
                      className="option-item"
                      onClick={() => handleSelectOption("category", "twinRoom")}
                    >
                      {t("categories.twinRoom")}
                    </div>
                    <div
                      className="option-item"
                      onClick={() =>
                        handleSelectOption("category", "tripleRoom")
                      }
                    >
                      {t("categories.tripleRoom")}
                    </div>
                    <div
                      className="option-item"
                      onClick={() =>
                        handleSelectOption("category", "familyRoom")
                      }
                    >
                      {t("categories.familyRoom")}
                    </div>
                    <div
                      className="option-item"
                      onClick={() =>
                        handleSelectOption("category", "suiteRoom")
                      }
                    >
                      {t("categories.suiteRoom")}
                    </div>
                    <div
                      className="option-item"
                      onClick={() =>
                        handleSelectOption("category", "deluxeRoom")
                      }
                    >
                      {t("categories.deluxeRoom")}
                    </div>
                  </>
                )}
                {/* {isTourPage && (
        ...
        <div
          className={`filter-group ${openDropdown === "price" ? "raise-z-index" : ""}`}
        >
          ...
          <div className="input-wrapper position-relative">
            ...
            >
              ...
              <ChevronDown
                size={18}
                className={`select-arrow-transition ${openDropdown === "price" ? "rotate-arrow" : ""}`}
              />
            </div>

            <div
              className={`custom-dropdown-options-box shadow-lg ${openDropdown === "price" ? "open-expanded" : ""}`}
            >
              <div
                className="option-item"
                onClick={() => handleSelectOption("price", "all")}
              >
                {t("prices.all")}
              </div>
              {isVehiclePage && (
                <>
                  <div
                    className="option-item"
                    onClick={() => handleSelectOption("price", "0-2000")}
                  >
                    {t("prices.below2k")}
                  </div>
                  <div
                    className="option-item"
                    onClick={() => handleSelectOption("price", "2000-10000")}
                  >
                    {t("prices.2kTo10k")}
                  </div>
                  <div
                    className="option-item"
                    onClick={() => handleSelectOption("price", "10000-999999")}
                  >
                    {t("prices.above10k")}
                  </div>
                </>
              ...
              {isHotelPage && (
                <>
                  ...
                  <div
                    className="option-item"
                    onClick={() => handleSelectOption("price", "2000-7000")}
                  >
                    {t("prices.2kTo7k")}
                  </div>
                  <div
                    className="option-item"
                    onClick={() => handleSelectOption("price", "7000-999999")}
                  >
                    {t("prices.above7k")}
                  </div>
                </>
              ...
              {isTourPage && (
                <>
                  <div
                    className="option-item"
                    onClick={() => handleSelectOption("price", "0-5000")}
                  >
                    {t("prices.below5k")}
                  </div>
                  <div
                    className="option-item"
                    onClick={() => handleSelectOption("price", "5000-15000")}
                  >
                    {t("prices.5kTo15k")}
                  </div>
                  <div
                    className="option-item"
                    onClick={() => handleSelectOption("price", "15000-999999")}
                  >
                    {t("prices.above15k")}
                  </div>
                </>
...
export const KumbhCountdown = ({ targetDate, isActive = true }) => {
  ...
  if (isExpired) {
    return (
      <div className="countdown-expired text-white h1 text-center">
        {t("expiredText")}
      </div>
    );
  }
  ...
  const CountdownBlock = ({ value, labelKey, fallbackLabel }) => {
    ...
    const formattedValue = String(value).padStart(
      labelKey === "days" ? 1 : 2,
      "0",
    );
            ...
            <div className="festival-card-wrapper snap-start group">
              ...
              <div className="festival-card festival-card-relative glass-card inner-glow position-relative p-4 d-grid rounded-4 transition-card">
                ...
                <div className="festival-date d-flex align-items-end gap-3">
                  <span className="festival-day fw-bold">{date.day}</span>

                  <div className="d-flex flex-column">
                    <span className="festival-month text-uppercase">
                      {t(date.monthKey)}
                    </span>
                    <span className="festival-year">{date.year}</span>
                  </div>
                </div>
                ...
                <div className="festival-info-popup position-absolute w-100 h-100 rounded-4 d-flex flex-column justify-content-center p-4">
                  <h5 className="mb-2 small primery-color">
                    {date.day} {t(date.monthKey)} {date.year}
                  </h5>
                  <span className="popup-desc mb-2 small text-white-50">
                    {t(date.dateOccationKey)}
                  </span>
                  <h4 className="popup-title mb-2 text-capitalize">
                    {t(date.titleKey)}
                  </h4>
                  <p className="popup-desc m-0 small text-white-50">
                    {t(date.descKey)}
                  </p>
                </div>
              </div>
...
// popup common data showing model
export const CommonPopup = ({ data, isOpen, onClose }) => {
  const tModal = useTranslations("CommonModal");

  // Extract all potential fields
  const title = data?.title || data?.name || "";
  const subtitle = data?.subtitle || "";
  const mainDescription = data?.description || data?.desc || "";
  const historyText = data?.history || "";
  const routeText = data?.route || "";
  const transportText = data?.transport || "";

  // Check if this card uses specific structured fields
  const hasStructuredData =
    historyText || routeText || transportText || data?.routeLink;
  ...
  return (
    ...
    >
      <Modal.Header className="border-0 px-4 py-3 modal-header justify-content-between align-items-center">
        <div>
          <Modal.Title className="text-dark fw-semibold mb-0">
            {title}
          </Modal.Title>
          {subtitle && (
            <span className="text-primary small fw-medium d-block mt-1">
              {subtitle}
            </span>
          )}
        </div>
        ...
        <button
          type="button"
          onClick={onClose}
          className="btn-close bg-transparent border-0 d-flex align-items-center justify-content-center"
      ...
      <Modal.Body className="p-4">
        {/* CASE A: Standard General Description (used for Rituals, About, Features) */}
        {!hasStructuredData && mainDescription && (
          <div className="mb-3">
            <p className="card-text text-muted lh-base">{mainDescription}</p>
          </div>
        )}

        {/* CASE B: Structured Content (used for Destinations / Tours) */}
        {hasStructuredData && (
          <>
            {/* Direct Description if present alongside structured data */}
            {mainDescription && (
              <div className="mb-3">
            ...
            {historyText && (
              <div className="mb-3">
                <h5 className="primery-color model-head">
                  {tModal("historyHeader")}
                </h5>
            ...
            {/* 3. Transport & Booking Section */}
            {(transportText || data?.routeLink) && (
              <div className="mb-3">
                <h5 className="primery-color model-head">
                  {tModal("transportHeader")}
                </h5>
